...
function useFormInput({
   ...
   }, [setIsValid, validation, name, value, handleError])
...
function validate(value, validation) {
   ...
   fields.forEach(field => {
      const { rule, props = null } = field
      switch (rule.trim()) {
         case 'isRequired':
            isValid = value && props ? true : false
            break
         default:
            if (isValid) {
               switch (typeof props) {
                  case 'boolean':
                     isValid = props && validator[rule](value)
                     break
                  case 'object':
                     if (props === null) {
                        isValid = false
                     } else {
                        let propsArray = Object.values(props)
                        propsArray.splice(0, 0, value)
                        isValid = validator[rule].apply(null, propsArray)
                     }
                     break
                  default:
                     isValid = validator[rule](value, props)
               }
            }
      }
   })
...
function useForm(defaultValues, invalidAttr = { error: true }) {
